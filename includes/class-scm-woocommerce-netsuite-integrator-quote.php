...
class SCM_WC_Netsuite_Integrator_Quote extends SCM_WC_Netsuite_Integrator_Service {
	...
	public function test_process_netsuite($order){
		...
		foreach($order->order_items as $itemKey => $webOrderItem){
			...
			if ($productSearchResponse === 0) {
				...
				if($productSearchResponse===FALSE) {continue;}
		...
		SCM_WC_Netsuite_Integrator::log_action('estimate_items_details', print_r($estimateItemDetails, true));
		...
		SCM_WC_Netsuite_Integrator::log_action('estimate_items', print_r($estimateItems, true));
	...
	*/
	...
	static function compareCountryCode($code){
		$codesArray = array(
			'AF'	=>	'_afghanistan',
			'AX'	=>	'_alandIslands',
			'AL'	=>	'_albania',
...
function test_function(){
	...
	$order = $WC_NIQ->get_order_details(2120);
	print_r($order); exit();
